  ...
  const car = req.body.car
	...
	try {
	  ...
	  try {
	      ...
	      result = await contract.submitTransaction('UpdateCar', car.ID, car.Color, car.Size, car.Owner, car.Price, car.Make, car.OwnerHistory, car.Validated, ['buying', userID]);
	      console.log('Car updated');
	      if (`${result}` !== '') {
	        console.log(`updated car: ${prettyJSONString(result.toString())}`);
	      }

				res.send('set to buying')
    } finally {
      gateway.disconnect();
    }
